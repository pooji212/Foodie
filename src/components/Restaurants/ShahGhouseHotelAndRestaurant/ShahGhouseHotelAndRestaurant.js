...
import SGHR from './ShahGhouseHotelAndRestaurant.json'

function ShahGhouseHotelAndRestarant({ cart, addToCart, removeItem }) {
    return (
        <div className='container bg-white'>
            <div className='sticky-top bg-white'>
                <h1 className='mt-3'>{SGHR.restaurantName}</h1>
                <div className="m-0 text-muted">{SGHR.foodStyle}</div>
                <div className="m-0 text-muted">{SGHR.location}</div>
                <div><span className="m-0 text-danger" style={{ marginLeft: '10px' }}>Open Now : </span>{SGHR.openingTime} - {SGHR.closingTime} (Today)</div>
...
export default ShahGhouseHotelAndRestarant