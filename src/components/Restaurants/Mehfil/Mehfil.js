...
import MehfilRestaurant from './Mehfil.json'

function Mehfil({cart,addToCart,removeItem}) 
{       
    return(
        <div className='container bg-white'>            
            <div className='sticky-top bg-white'>
                <h1 className='mt-3'>{MehfilRestaurant.restaurantName}</h1>
                <div className="m-0 text-muted">{MehfilRestaurant.foodStyle}</div>
                <div className="m-0 text-muted">{MehfilRestaurant.location}</div>
                <div><span className="m-0 text-danger" style={{ marginLeft: '10px' }}>Open Now : </span>{MehfilRestaurant.openingTime} - {MehfilRestaurant.closingTime} (Today)</div>
