...
function Lucky_Restaurant({ cart, addToCart, removeItem }) {
    return (
        <div className='container bg-white'>
            <div className='sticky-top bg-white'>
                <h1 className='mt-3'>{LuckyRestaurant.restaurantName}</h1>
                <div className="m-0 text-muted">{LuckyRestaurant.foodStyle}</div>
                <div className="m-0 text-muted">{LuckyRestaurant.location}</div>
                <div><span className="m-0 text-danger" style={{ marginLeft: '10px' }}>Open Now : </span>{LuckyRestaurant.openingTime} - {LuckyRestaurant.closingTime} (Today)</div>
...
export default Lucky_Restaurant